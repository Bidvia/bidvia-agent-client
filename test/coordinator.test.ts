...
test('runMultiBusinessChainCoordinatorPreHandoff rebuilds the approval side of the external handoff boundary from runtime connection truth', async () => {
  const calls: string[] = [];
  const client = {
    async createListing(input) {
      calls.push(`createListing:${input.listingId}`);
    ...
    async createConnectionRequest(input) {
      calls.push(`createConnectionRequest:${input.sourceMatchId}`);
      return { approval_request_id: 'approval-runtime-1' };
    },
    async approveConnectionRequest(input) {
      calls.push(`approveConnectionRequest:${input.approvalRequestId}`);
      return { ok: true };
  ...
  assert.deepEqual(calls, [
    ...
    'generateMatchCandidates:listing-1',
    'createConnectionRequest:match-1',
    'approveConnectionRequest:approval-runtime-1',
  ]);
  assert.deepEqual(result.externalHandoffBoundary, {
...
test('runMultiBusinessChainCoordinatorWithExplicitHandoff carries the runtime approval boundary into post-handoff results', async () => {
  const calls: string[] = [];
  const client = {
    async createListing(input) {
      calls.push(`createListing:${input.listingId}`);
    ...
    async exportOpportunityPackage(input) {
      calls.push(`exportOpportunityPackage:${input.opportunityId}`);
      return { package_id: 'pkg-runtime-1' };
    },
    async createCommercialAction(input) {
      calls.push(`createCommercialAction:${input.subjectId}`);
  ...
  } as PreHandoffCoordinatorClient & PostHandoffCoordinatorClient;
  ...
  assert.deepEqual(calls, [
    ...
    'exportOpportunityPackage:opportunity-1',
    'createCommercialAction:pkg-1',
    'policyCheckCommercialAction:commercial-action-runtime-1',
    'requestCommercialActionApproval:approval-1',
    'executeCommercialAction:receipt-1',
  ]);
  assert.deepEqual(result.preHandoff.externalHandoffBoundary, {
    ...plan.externalHandoffBoundary,
    approvalRequestId: 'approval-runtime-1',
  });
  assert.deepEqual(result.postHandoff.externalHandoffBoundary, {
    ...plan.externalHandoffBoundary,
    approvalRequestId: 'approval-runtime-1',
  });
});
