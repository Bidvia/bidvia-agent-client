...
test('executeIndustryUniverseScenario returns verification bundle plus execution result', async () => {
  const { calls, fetchStub } = createFetchStub();
  const client = new BidviaClient({
    baseUrl: 'http://127.0.0.1:8787',
    context: {
  ...
  const result = await executeIndustryUniverseScenario(client, plan);
