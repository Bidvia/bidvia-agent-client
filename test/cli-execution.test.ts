...
test('runCli describes the companion bundle as additive packaging around the stdio MCP path after writing to disk', async () => {
  ...
  assert.equal(
    output.operatorNotes.executionBoundary,
    'Bundle/bootstrap only: OpenClaw stays config and packaging around the local stdio MCP server at `bidvia mcp-server`, where Bidvia execution actually runs.',
  );
  assert.equal(
