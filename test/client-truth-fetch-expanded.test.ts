...
test('BidviaClient broader semantic, pricing, and asset truth-fetch reads guard tenant context before fetch', async () => {
  const calls: Array<{ input: RequestInfo | URL; init?: RequestInit }> = [];
  const client = new BidviaClient({
    baseUrl: 'http://127.0.0.1:8787',
    context: {
