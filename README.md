...
Feature-branch bilateral draft entry (2026-09-07): createChemicalRfq / listChemicalRfqs / getChemicalRfq / offerChemicalRfq / decideChemicalRfq / downloadChemicalOfferDocument connect independent buyer/seller administrator workspaces to persistent Core draft negotiation. Seller explicitly shares pinned originals; buyer decides only the latest offer/version/digest. ACCEPT_FOR_DRAFT retains a review draft, never signs, reserves inventory or creates an order. [Executable role-action/export example](examples/chemical-trade.ts) runs after build with BIDVIA_CHEMICAL_BASE_URL, BIDVIA_CHEMICAL_TENANT_ID and the current role's BIDVIA_CHEMICAL_ADMIN_SESSION_ID (loopback only). Export verifies SHA256 and refuses overwrite. Paired Core docs/business-scenarios/cn-vn-soda-ash/runtime-bilateral-draft.md specifies required existing pricing basis, request JSON, statuses and additive development migration. No Console/MCP/main CLI expansion or npm release.

Feature-branch original-document entry (2026-09-07): uploadChemicalDocument / getChemicalDocumentDossier / getChemicalDocument / downloadChemicalDocument / reviewChemicalDocument connect operator workspace sessions to Core chemical-file intake. Download verifies the original-byte SHA256; review requires a different underlying admin account. Claims are human-entered, not OCR; test originals remain test-only and no supplier/import/shipment approval is implied. Use [the executable file intake example](examples/chemical-document-intake.ts) for local upload, detail, dossier, non-overwriting download and explicit review after npm run build. It requires BIDVIA_CHEMICAL_BASE_URL, BIDVIA_CHEMICAL_TENANT_ID and BIDVIA_CHEMICAL_ADMIN_SESSION_ID; paired Core docs/business-scenarios/cn-vn-soda-ash/runtime-documents.md documents metadata and additive development migration. No new main CLI/MCP command or npm publication is claimed.

Feature-branch chemical business entry (2026-09-07): createListing now accepts typed chemicalSpecification; updateListing edits a draft quantity/specification; previewChemicalQuotation binds commercialTerms to expectedListingVersion. Admin-enabled listing create/activate/match use session-only wire authority. The preview is an operator-workspace-only, nonpersistent TEST quotation/contract with unverified quality-document declarations, not a signed contract or shipment permission. See [the loopback-only runnable example](examples/cn-vn-chemical-listing.ts) and paired Core docs/business-scenarios/cn-vn-soda-ash/runtime-listing-quotation.md. Run node dist/examples/cn-vn-chemical-listing.js after build with its documented BIDVIA_CHEMICAL_* environment inputs; it writes one unverified TEST draft, never activates it, and leaves price/parties unset. These helpers are SDK additions, not new CLI/MCP commands or an npm release.
