...
| Event / notification plane | packet-grounded-execution | Notification detail reads are payload-grounded and acknowledgement is the only packet-grounded execution helper in this wave. Delivery, retry, and expiry remain compatibility-only. | Keep replay or other still-missing semantics fail-closed unless Core freezes them. |
